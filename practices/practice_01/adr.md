...
4. Приводить ответ к контракту OUT-1 в сервисе (`ReviewService.review`): `summary`, `risks` (≤ 3, с `file`, `line`, `evidence`, `risk`), `checks`.
...
| Формировать OUT-1 в API-слое | Размывает ответственность и требует понимать внутренние детали сервиса; выше риск дублирования |
| Ввести Presenter-слой для шейпинга OUT-1 | Новая абстракция на раннем этапе, усложняет внедрение без дополнительных выгод для Практики 2 |
